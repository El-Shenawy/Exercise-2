...
void main() {
  group('ExchangeRatePage', () {
    ...
    testWidgets('should show loading animation when applying data', (WidgetTester tester) async {
      when(exchangeRateController.isDataProcessing).thenReturn(true.obs);
      await tester.pumpWidget(MaterialApp(home: ExchangeRatePage()));
    ...
    testWidgets('should call exchange rate controller when applying data', (WidgetTester tester) async {
      when(exchangeRateController.isDataProcessing).thenReturn(false.obs);
      await tester.pumpWidget(MaterialApp(home: ExchangeRatePage()));
      await tester.tap(find.text('Apply'));
      verify(exchangeRateController.startDateController.text).called(1);
      verify(exchangeRateController.endDateController.text).called(1);
      verify(exchangeRateController.baseCurrencyController.text).called(1);
      verify(exchangeRateController.targetCurrencyController.text).called(1);
    });
    ...
    testWidgets('should show exchange rates when data is fetched', (WidgetTester tester) async {
      when(exchangeRateController.isDataProcessing).thenReturn(false.obs);
      when(exchangeRateController.exchangeRatesList).thenReturn([
        ExchangeRate(date: startDate, rate: 1.0),
        ExchangeRate(date: startDate.add(Duration(days: 1)), rate: 1.1),
        ExchangeRate(date: startDate.add(Duration(days: 2)), rate: 1.2),
      ] as RxList<ExchangeRate?>);

    testWidgets('should show error when data fetch fails', (WidgetTester tester) async {
      when(exchangeRateController.isDataProcessing).thenReturn(false.obs);
      await tester.pumpWidget(MaterialApp(home: ExchangeRatePage()));
